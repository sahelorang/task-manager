import React from "react";
import { BiCalendarAlt } from "react-icons/bi";
import "./Task.css";
import { FaRegCalendarCheck } from "react-icons/fa";
import { FiCheckCircle } from "react-icons/fi";
import { HiOutlineClipboardCheck } from "react-icons/hi";
import { MdOutlineClose } from "react-icons/md";
import useAuth from "src/hooks/useAuth";

const Task = ({ taskName, category, rate, complete, task, setAllTask }) => {
  const { user } = useAuth();

  const handleCategory = () => {
    switch (category) {
      case "Design System":
        return (
          <span className="Design-Category Category fw-bolder py-1 px-2">
            Design System
          </span>
        );
      case "Branding":
        return (
          <span className="Branding-category Category fw-bolder py-1 px-2">
            Branding
          </span>
        );
      case "Desktop":
        return (
          <span className="Desktop- fw-bolder Category py-1 px-2">Desktop</span>
        );
      case "UX & Research":
        return (
          <span className="UX-Category Category fw-bolder py-1 px-2">
            UX & Research
          </span>
        );
      default:
        return null;
    }
  };

  const getCurrentDate = (separator = ".") => {
    let newDate = new Date();
    let year = newDate.getFullYear();
    let month = newDate.getMonth();
    let date = newDate.getDate();

    return `${year}${separator}${
      month < 10 ? `0${month}` : `${month}`
    }${separator}${date}`;
  };

  const handleUpdateToToDo = (taskId) => {
    const allTask = localStorage.Tasks;
    if (allTask) {
      const parsedTasks = JSON.parse(allTask);
      parsedTasks[user].forEach((task) => {
        if (task.id === taskId) {
          task.status = "ToDo";
        }
        localStorage.setItem("Tasks", JSON.stringify(parsedTasks));
        setAllTask(parsedTasks[user]);
      });
    }
  };

  const handleUpdateToProgress = (taskId) => {
    const allTask = localStorage.Tasks;
    if (allTask) {
      const parsedTasks = JSON.parse(allTask);
      parsedTasks[user].forEach((task) => {
        if (task.id === taskId) {
          task.status = "InProgress";
        }
        localStorage.setItem("Tasks", JSON.stringify(parsedTasks));
        setAllTask(parsedTasks[user]);
      });
    }
  };

  const handleUpdateToDone = (taskId) => {
    const allTask = localStorage.Tasks;
    if (allTask) {
      const parsedTasks = JSON.parse(allTask);
      parsedTasks[user].forEach((task) => {
        if (task.id === taskId) {
          task.status = "Done";
        }
        localStorage.setItem("Tasks", JSON.stringify(parsedTasks));
        setAllTask(parsedTasks[user]);
      });
    }
  };

  const closeCard = (taskId) => {
    const allTask = localStorage.Tasks;
    if (allTask) {
      const parsedTasks = JSON.parse(allTask);
      const NewTasks = parsedTasks[user].filter((task) => task.id !== taskId);
      parsedTasks[user] = NewTasks;
      localStorage.setItem("Tasks", JSON.stringify(parsedTasks));
      setAllTask(NewTasks);
    }
  };

  return (
    <div>
      <div className="task-bg shadow-sm mb-2 p-3">
        <div className="d-flex justify-content-between align-items-center">
          <span className="name-task d-block fw-bold fs-6">{taskName}</span>
          <MdOutlineClose onClick={() => closeCard(task.id)} />
        </div>
        <div className="d-flex justify-content-between align-items-end py-3 pt-1  border-bottom">
          <div>{handleCategory()}</div>
          <div>
            <img
              alt="icon"
              className="rounded-circle avatar-container-item img"
              src="data:image/jpeg;base64,"
            />
            <img
              alt="icon"
              className="rounded-circle avatar-container-item"
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQSNPAaw6wL5WLn5xjWZ0MNWAfZ2Av3LDJFpA&usqp=CAU"
            />
          </div>
        </div>
        <div className="d-flex justify-content-between  py-2 pb-0">
          <div className="d-flex align-items-center">
            {complete ? (
              <FiCheckCircle className="icon-task-color fs-6 me-1" />
            ) : (
              <HiOutlineClipboardCheck className="icon-task-color fs-5" />
            )}
            <span className="font-footer-task fw-semibold  ms-1">
              {complete ? "complete" : rate}
            </span>
          </div>
          <div className="d-flex align-items-center">
            {complete ? (
              <FaRegCalendarCheck className="icon-task-color fs-6 " />
            ) : (
              <BiCalendarAlt className="icon-task-color fs-5 " />
            )}

            <span className="font-footer-task  fw-semibold ms-2">
              {getCurrentDate()}
            </span>
          </div>
        </div>
      </div>
      <div className="d-flex justify-content-between align-items-center mb-4 mt-3 ">
        <button className="btn-task btn " onClick={() => closeCard(task.id)}>
          close
        </button>
        <button
          className="btn-task btn"
          onClick={() => handleUpdateToToDo(task.id)}
        >
          ToDo
        </button>
        <button
          className="btn-task btn"
          onClick={() => handleUpdateToProgress(task.id)}
        >
          InProgress
        </button>
        <button
          className="btn-task btn"
          onClick={() => handleUpdateToDone(task.id)}
        >
          Done
        </button>
      </div>
    </div>
  );
};

export default Task;
